...
class SummonerViewController: UIViewController {
    ...
    fileprivate let saveStats = SaveStats(character: Classes.sum.rawValue)
    fileprivate let weapons = Weapon(belongsTo: Classes.sum)
    fileprivate let armors = Armor(belongsTo: Classes.sum)
...
extension SummonerViewController: SaveDataDelegate {
    func saveData(_ isActive: Bool, clas: Classes) {
        if calculateButtonOutlet.isEnabled == true && clas == Classes.sum {
            saveStats.saveData(rebirthInput.text, key: InputStats.rebirth.key)
            saveStats.saveData(levelInput.text, key: InputStats.level.key)
            saveStats.saveData(fruitStatsInput.text, key: InputStats.creaton.key)
            saveStats.saveData(goldenCrownPoints, key: InputStats.goldenCrown.key)

...
class DarkKnightViewController: UIViewController {
    ...
    fileprivate let saveStats = SaveStats(character: Classes.dk.rawValue)
    fileprivate let weapons = Weapon(belongsTo: Classes.dk)
    fileprivate let armors = Armor(belongsTo: Classes.dk)
...
extension DarkKnightViewController: UITextFieldDelegate {
    func updateStats(_ textField: String?, stat: Stats) {
...
extension DarkKnightViewController: SaveDataDelegate {
    func saveData(_ isActive: Bool, clas: Classes) {
        if calculateButtonOutlet.isEnabled == true && clas == Classes.dk {
            saveStats.saveData(rebirthInput.text, key: InputStats.rebirth.key)
            saveStats.saveData(levelInput.text, key: InputStats.level.key)
            saveStats.saveData(fruitStatsInput.text, key: InputStats.creaton.key)
            saveStats.saveData(goldenSwordPoints, key: InputStats.goldenSword.key)
